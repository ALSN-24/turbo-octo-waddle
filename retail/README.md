...
| Area | Before | After |
|---|---|---|
| Database | Supabase / PostgreSQL | MySQL 8.0+ via `mysql2` |
| Authentication | Supabase Auth (magic link) | bcrypt passwords + HS256 JWT cookie |
| Sessions | Supabase SSR cookie | `crm_session` HttpOnly cookie, 7-day expiry |
| Data access | `supabase.from().select()` | `fetch('/api/...')` → API routes → raw SQL |
| Route protection | Supabase session refresh | Edge JWT verification in `middleware.ts` |
| **Removed packages** | `@supabase/ssr`, `@supabase/supabase-js`, `@vercel/analytics` | *(deleted)* |
...
| File | Action | Notes |
|---|---|---|
| `package.json` | REPLACE | Added `mysql2`, `bcryptjs`, `jose`, `eslint`. Removed `@supabase/*`. |
...
| `lib/supabase/` | DELETE | Entire directory. |
...
| File | Action | Notes |
|---|---|---|
| `components/crm-layout.tsx` | REPLACE | Accepts `userEmail` / `userName` props instead of Supabase `User`. |
| `components/crm-sidebar.tsx` | REPLACE | Logout calls `/api/auth/logout`. Error-tolerant redirect. |
| `components/customers-content.tsx` | REPLACE | Supabase → `fetch()`. Toast feedback. Loading states. Validation. |
